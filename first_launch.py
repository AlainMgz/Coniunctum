...
from modules.add_new_node import add_new_full_node
...
# Setup accordingly to the user's role in the network
	...
	if role == 1:
		...
		run_temp_app = call("shell_scripts/start_temp.sh", shell=True)
		sleep(5)
		source_ip = get_source_ip()
		res = add_new_full_node(source_ip)
		if res:
			with open("data/nodes.json") as nodes_file:
				...
				for n in nodes_data['nodes']:
					...
					if n != f"http://{ip_local}:56230":
						if n != f"http://{source_ip}:56230":
							res2 = add_new_full_node(n[7:19])
			stp_temp_app = call("shell_scripts/stop_temp.sh", shell=True)
